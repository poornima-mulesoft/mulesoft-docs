...
|<<runtime-mgr application delete>>| Deploy a new application
...
[source,bash]
----
$ runtime-mgr application delete  [options] <name> <zipfile>
----
...
|===
|Value |Description
|http | Specifies the Load balancer HTTP behavior. It can be set to `on` `off` or `redirect`.
|clientCertificate | Client certificate file
|verificationMode  | Specifies the client verification modeñ. It can be set to `on` (verify always) `off` (don´t verify) or `opional` (Verification optional).
|crl | Certificate Revocation List file
|tlsv1  | Supports TLSv1 in addition to TLSv1.1 and TLSv1.2
|===
