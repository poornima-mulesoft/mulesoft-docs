...
|<<cloudhub load-balancer mappings describe  [certificateName]>>| Lists the proxy mapping rules for a Load balancer. If no certificateName is given, the mappings for the default SSL endpoint are shown
...
[NOTE]
The whitelist works at the load balancer level, not at the common name (CN) certificate level. Make sure you only pass IP addresses.
...
----
This command creates a VPC using the name in <name> in the region specified in <region> and setting a firewall rule for the IP, or range of IP addresses, passed in <cidrBlock>.
