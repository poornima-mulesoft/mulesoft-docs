...
Files need to be pem encoded and not encrypted.
...
|===
|Value |Description
|http | Specifies the Load balancer HTTP behavior. It can be set to `on` `off` or `redirect`. (no escucha en http) on contesta dependiendo de tu  sslendpoint default
|clientCertificate | Client certificate file
