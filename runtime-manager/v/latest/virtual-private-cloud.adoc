...
* Host your applications in a VPC and take advantage of its link:/runtime-manager/cloudhub-load-balancer[load balancer features]
...
Each VPC can host one or many link:/runtime-manager/cloudhub-networking-guide#load-balancing[load balancers] with a _DNS A record_ that you can add to your own DNS CNAME record to point to. This way you can configure your VPC´s load balancer to handle requests to your own domain.
