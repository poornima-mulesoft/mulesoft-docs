...
As an organization administrator, you can create a VPC and share it to any business group within your main organization, however once the VPC is inherited by a business group, only the owner of said business group can operate the VPC (i.e. set firewall rules, assign it as default to an environment within that business group, etc). +
